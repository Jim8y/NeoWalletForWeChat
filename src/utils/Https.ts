...
export default class Https {
  ...
  static apiaggr: string =
      'https://apiscan.nel.group/api/testnet';  //"http://seed1.nether.top:12122"//
  ...
  static makeRpcUrl(url: string, method: string, ..._params: any[]) {
    if (url[url.length - 1] != '/' && !url.includes('apiscan')) url = url + '/';
    var urlout = url + '?jsonrpc=2.0&id=1&method=' + method + '&params=[';
    for (var i = 0; i < _params.length; i++) {
      urlout += JSON.stringify(_params[i]);
      if (i != _params.length - 1) urlout += ',';
    }
    urlout += ']';
    return urlout;
  }
  ...
  static async makeaddrpost(body) {
    var result = await Request.wxRequest(
        {'method': 'post', 'body': body},
        this.proxy_server + 'neowallet/index.php');
    return result;
  }
  ...
  static async api_getCoinPrice(coin) {
    try {
      return await Request.wxRequest(
          {'method': 'get'}, this.priceHost + coin + '/?convert=CNY');
    } catch (error) {
  ...
  static async rpc_postRawTransaction(data: Uint8Array) {
    var postdata =
        this.makeRpcPostBody('sendrawtransaction', Helper.toHexString(data));
    var result = await Request.wxRequest(
        {
          'method': 'post',
          'body': {'tx': JSON.stringify(postdata), 'server': this.api}
        },
        this.proxy_server + 'proxy.php');
    console.log(result);
    try {
  ...
  static async rpc_getInvokescript(scripthash: Uint8Array): Promise<any> {
    var str = this.makeRpcUrl(
        this.api, 'invokescript', Helper.toHexString(scripthash));
  ...
  static async rpc_getRawTransaction(txid: string) {
    ...
    var result = await Request.wxRequest(
        ...
        this.proxy_server + 'proxy.php');
    // var result = await Request.wxRequest({ "method": "post",
    // "body":JSON.stringify(postdata)}, this.rpc);
    try {
      return result['result'];
    } catch (error) {
      // console.log(error);
   ...
   */
  static async rpc_getAddressTXs(
      addr: string, max: number = 20, page: number = 1) {
    var postdata = this.makeRpcPostBody('getaddresstxs', addr, max, page);
    var result = await Request.wxRequest(
        {'method': 'post', 'body': JSON.stringify(postdata)}, this.api);
    try {
      return result['result'];
    } catch (error) {
      return null;
  ...
  static async api_getnep5Balance(address: string) {
    ...
    var str =
        Https.makeRpcUrl(Https.api, 'getallnep5assetofaddress', address, 1);
    // console.log(str);
    var result = await Request.Request({'method': 'get'}, str);
    try {
      return result['result'];
  ...
  static async rpc_getStorage(scripthash: Uint8Array, key: Uint8Array):
      Promise<string> {
    // var str = this.makeRpcUrl(this.api, "getstorage",
    // Helper.toHexString(scripthash), Helper.toHexString(key)); var result =
    // await fetch(str, { "method": "get" }); var json = await result.json(); if
    // (json["result"] == null)
    //     return null;
   ...
   * @param pageindex 页下标
   */
  static async gettransbyaddress(
      address: string, pagesize: number = 20, pageindex: number = 1) {
    var postdata = Https.makeRpcPostBody(
        'gettransbyaddress', address, pagesize, pageindex);

    var result = await Request.wxRequest(
        {
          'method': 'post',
          'body': {'tx': JSON.stringify(postdata), 'server': this.apiaggr}
        },
        this.proxy_server + 'proxy.php');
    console.log(result)
    try {
  ...
  static async rechargeandtransfer(data1: Uint8Array, data2: Uint8Array) {
    var postdata = Https.makeRpcPostBody(
        'rechargeandtransfer', Helper.toHexString(data1),
        Helper.toHexString(data2));
    var result = await Request.wxRequest(
        {
          'method': 'post',
          'body': {'tx': JSON.stringify(postdata), 'server': this.apiaggr}
  ...
  static async setnnsinfo(address: string, name: string, time: number) {
    var str =
        Https.makeRpcUrl(Https.apiaggr, 'setnnsinfo', address, name, time);
    //  var result = await fetch(str, { "method": "get" });
    var result = await Request.Request({'method': 'get'}, str);
    // var result = await Request.wxRequest({ "method": "post", "body": { 'tx':
    // JSON.stringify(postdata), 'server': this.apiaggr } }, this.proxy_server +
    // "proxy.php"); var json = await result.json();
  ...
  static async getnnsinfo(...params): Promise<string[]> {
    ...
    var result = await Request.wxRequest(
        ...
        this.proxy_server + 'proxy.php');
    //  var result = await fetch(Https.apiaggr, { "method": "post", "body":
    //  JSON.stringify(postdata) });
    // var json = await result.json();
    if (result['result'] == null) return null;
    var r = result['result'] return r;
  }
  ...
  static async delnnsinfo(domain: string) {
    ...
    // var json = await result.json();
    if (result['result'] == null) return null;
    var r = result['result'][0]['result'] return r;
  }
   ...
   * @param pagesize
   */
  static async api_getBidDetail(
      id: string, currentpage: number, pagesize: number) {
    var postdata = Https.makeRpcPostBody(
        'getbiddetailbydomain', id, currentpage, pagesize);
    console.log(postdata)
    var result = await Request.wxRequest(
        {
          'method': 'post',
  ...
  static async getDomainState(address: string, id: string) {
    ...
    // var result = await fetch(WWW.apiaggr, { "method": "post", "body":
    // JSON.stringify(postdata) });
    var result = await Request.wxRequest(
        {
          'method': 'post',
  ...
  static async getNotify(txid: string) {
    ...
    // var result = await fetch(WWW.api, { "method": "post", "body":
    // JSON.stringify(postdata) });
    var result = await Request.wxRequest(
        {'method': 'post', 'body': JSON.stringify(postdata)}, this.api);
    var json = await result.json();
  ...
  static async hascontract(txid: string) {
    ...
    var result = await Request.wxRequest(
        ...
        this.proxy_server + 'proxy.php');
    // var result = await fetch(WWW.apiaggr, { "method": "post", "body":
    // JSON.stringify(postdata) }); var json = await result.json();
    var r = result['result'][0];
    return r;
  }
  ...
  /**
   * 获取CGAS合约可用于兑换GAS的UTXO
   * @param address 地址
   * @param count 当前页码
   * @param pagesize 条数
   */
  static async getavailableutxos(address: string, count: number) {
    var postdata = Https.makeRpcPostBody('getavailableutxos', address, count);
    var result = await Request.wxRequest(
        {
          'method': 'post',
          'body': {'tx': JSON.stringify(postdata), 'server': this.apiaggr}
   ...
   */
  static async notify(
      txid, openid, addr, sendTime, type, amount, token, formId) {
    // 交易时间{{keyword1.DATA}}
    // 交易类型{{keyword2.DATA}}
    // 交易金额{{keyword3.DATA}}
    // 订单编号{{keyword4.DATA}}
    ...
    let body = {
      ...
      form_id: formId,
      'data': {
        'keyword1': {'value': sendTime, 'color': '#173177'},
        'keyword2': {'value': type + '交易', 'color': '#173177'},
        'keyword3': {'value': amount + type, 'color': '#173177'},
        'keyword4': {'value': txid, 'color': '#173177'},
        'keyword5': {'value': addr, 'color': '#173177'}
      }
    };
